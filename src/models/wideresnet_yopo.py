...
class Wide_ResNet(nn.Module):
    ...
    def forward(self, x, vx=None, feature=True):
        ...
        if self.sum_pool:
            ...
            out = F.avg_pool2d(out, 32) #input이 [3, 128, 128]여서 32으로 설정해줘야 shape가 맞음
            #out = F.avg_pool2d(out, 16) #input이 [3, 64, 64]여서 16으로 설정해줘야 shape가 맞음
            #out = F.avg_pool2d(out, 8) # 얘는 [3, 32, 32]일 경우만 작동
